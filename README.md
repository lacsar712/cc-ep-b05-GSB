...
4. 在详情页记录指标、选择产物类型（模型/数据集/日志/图）挂载产物，再 Complete（或 Abort）；输入枚举外的类型会被页面与服务端拒绝
...
- **产物类型枚举**：`AttachArtifact.artifact_type` 仅允许 `model` / `dataset` / `log` / `graph`（模型/数据集/日志/图）；其它值在页面与服务端都会被拦截并返回原因（422），非法挂载不产生事件
